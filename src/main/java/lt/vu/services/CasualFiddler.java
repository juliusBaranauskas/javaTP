...
@Alternative
@Model
public class CasualFiddler implements GameRatingFiddler {
    ...
    public void increaseBy(int id, int increase) {
        ...
        if (avgRating + increase >= 9)
            return;

        int i = 1;
        while (i < 1000) {
            if ((9*i + avgRating*ratingLen)/(ratingLen+i) >= increase + avgRating)
                break;
